...
- 光环识别页：使用 Fuyutsui 提供的定位点截取光环区域，并通过 `dHash + 模板匹配` 识别图标；可启用 500ms 实时扫描、手动扫描、刷新模板、打开模板目录，并在“识别结果 / 已有图标”之间切换查看和命名图标。启用识别后，已命名的识别光环会导入运行状态与光环列表，供模块条件引用；未命名图标只保存到 `tmp` 等待整理。
...
- `tmp/`：光环识别自动保存的未命名图标，文件名为 dHash；重复哈希会跳过。
...
- `启用识别`：开启后每 `500ms` 扫描一次，并把已命名的识别结果写入运行状态、状态页和模块条件可用字段。
- `手动扫描`：立即扫描一次，适合调试定位点和模板。
- `刷新模板`：重新读取 `auras` 和 `AurasHash.json`。
- `匹配距离`：控制 dHash 允许的最大距离。数值越小越严格，误识别更少但可能漏识别；数值越大越宽松，能匹配更多相似图标但误识别风险更高。
- 自动识别到但未命名的图标会保存到 `tmp`，方便之后在“已有图标”中查看、排序和改名。
