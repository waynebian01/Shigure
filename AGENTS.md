...
保存后 [Infrastructure/FuyutsuiKeymapConverter.cs](Infrastructure/FuyutsuiKeymapConverter.cs)`.UpdateFromClassMacros` 将 Lua 宏表按 dynamic（每项 40 槽）→ static → special 顺序转换为 `keymap/*.json`，把宏槽位映射到热键池（26 组左右修饰键 × 44 个基础键 = 1144 组合/职业；不含 F4 与 DELETE，避免 ALT-CTRL-DELETE 等系统级组合）。`DeriveSpellName` 解析 WoW 宏文本提取技能名。
