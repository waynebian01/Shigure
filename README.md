...
- `配置`：直接编辑项目 `Fuyutsui/class/*.lua` 中的 `ClassBlocks`，包括状态、光环、冷却（技能冷却与当前专精物品冷却）和队伍字段；玩家、目标、焦点及姓名板光环可用“玩家施放”控制是否追加 `PLAYER` 筛选，未勾选（或 Lua 中省略 `isPlayer`）时为 false；技能列表页可编辑 `Fuyutsui.spellsList` 中索引 1–100 的法术 ID、索引和名称，也可从技能数据库添加；物品列表页编辑职业级 `Fuyutsui.itemsList`（`[itemId] = { index, name }`，允许同名、禁止同 ID），并可从全量物品数据库添加。旧版稀疏索引格式只读，需先迁移到 `states/auras/spells/items/group` 格式。
...
触发键、发送模式、画面捕获方式、性能间隔、模块选择、默认模块、模块保存以及配置同步都会按需重启运行循环。窗口位置、大小、主界面横纵布局（两个方向分别保存窗口位置）、画面捕获方式、性能间隔、模块选择、按筛选条件保存的默认模块和表格列宽等 UI 状态保存在我的文档目录 `{MyDocuments}/Shigure/cache/window-state.json`；模块保存在 `{MyDocuments}/Shigure/module`。`cache/` 与模块数据都是本地数据，默认不提交到 Git。
...
Windows 的正式无边框捕获流程要求程序具有包身份、在包清单声明无边框能力，并由用户授权。Shigure 使用“外部位置（稀疏）MSIX”只提供身份，程序文件仍保留在普通可写目录，不改变现有配置和插件文件的读写方式。

开发机首次配置：

```powershell
.\Packaging\New-DevelopmentCertificate.ps1
.\Packaging\Build-SparsePackage.ps1 `
  -CertificatePath .\artifacts\certificate\Shigure.Development.pfx `
  -CertificatePassword Shigure-Development-Only `
  -Register
```

需要安装 Windows SDK 的 MSIX Packaging Tools（`MakeAppx.exe` 与 `SignTool.exe`）。注册完成后，从脚本输出的 `artifacts\sparse-package\publish\Shigure.exe` 启动，并在 Windows 首次弹出的授权窗口中允许无边框捕获。开发证书仅供本机测试；正式发布必须使用主题为 `CN=Arasaka Corporation` 的可信代码签名证书。直接运行 `dotnet run` 没有包身份，系统仍可能显示黄色边框。
...
- `45`：鼠标指向。
- `46`～`50`：首领 1～5。
- `51`～`55`：竞技场单位 `arena1`～`arena5`。

当前映射版本为 `UnitMappingVersion = 4`。版本 1 模块会迁移旧的 `31/34` 含义；版本 2 模块中的 `36/37` 会迁移为 unit 0，并分别补成 `channeling` / `nochanneling` 宏条件；版本 3 的保留单位 `31`～`35` 会迁移到 `41`～`45`。生成的 keymap 同样携带映射版本，确保新版团队槽 `36/37` 不会被当成旧宏条件。

像素状态中的“目标类型/焦点类型”会把 `arena1`～`arena5` 编码为 `51`～`55`，“其他单位”使用 `56`；友方仍在基础编号上加 `100`。
...
- `逻辑延迟 (ms)`：发送后暂停模块规则计算指定时间；画面扫描、触发键检测和界面刷新仍继续。
...
- 队友单位：没有独立的顶部选择器，所有条件由彼此独立、同时按“且”生效的筛选卡片配置。生命值和治疗吸收卡片可分别选择“生命值最低”或“治疗吸收最高”作为目标选择方式；光环拆为“光环存在”（有/没有某一个或所选任一光环）与“光环时长”（先选择一个光环，再筛选持续最长/最短）两个卡片。这三种数值目标选择互斥，数值相同时固定取正序首个。职责、驱散和光环存在共享正序/逆序方向；未选择数值目标时按该方向取首个，无相关筛选时默认正序。旧模块中的组合式单位选择器会在读取时迁移为等价筛选。队友单位可在规则目标中直接使用，也可用裸名称判断是否成功解析。
- 队友数量：与敌人数量一样使用彼此独立、同时按“且”生效的筛选卡片，支持生命值、治疗吸收、职责、驱散类型和光环；光环可选择“同时拥有光环（多选）”，要求单位拥有至少两个所选光环中的全部光环。只能用于条件。旧模块中的组合式数量选择器会在读取时迁移为等价筛选。
- 敌人数量：统计姓名板中满足生命值、光环、距离与战斗状态筛选的敌人；光环可选择“同时拥有光环（多选）”，要求单位拥有至少两个所选光环中的全部光环；选择“有某一个光环”时还可按该光环剩余时间大于/小于指定秒数筛选。该字段只能用于条件；基础范围只包含存在且距离大于 0 的姓名板。
- 平均血量：可选择队友或敌人作为统计对象。队友支持生命值、光环、职责筛选；敌人支持生命值、光环、距离、战斗筛选。结果四舍五入为整数，无匹配单位时为 `0`。
