...
- 队友数量：与敌人数量一样使用彼此独立、同时按“且”生效的筛选卡片，支持生命值、治疗吸收、职责、驱散类型和光环；只能用于条件。旧模块中的组合式数量选择器会在读取时迁移为等价筛选。
- 敌人数量：统计姓名板中满足生命值、光环、距离与战斗状态筛选的敌人；选择“有某一个光环”时还可按该光环剩余时间大于/小于指定秒数筛选。该字段只能用于条件；基础范围只包含存在且距离大于 0 的姓名板。
