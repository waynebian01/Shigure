...
3. **治疗吸收网格**：CountBars 下方最多 8 行、40 个单位，白条后的背景像素携带吸收值和单位编号。
...
| `groupList`、生命值与治疗吸收 | 最多 40 个单位的吸收网格 |
...
扫描从 CountBars 下一行开始，最多检查 8 行。每段先找到纯白起点，跳过连续白色，再读取第一个非白背景像素：
